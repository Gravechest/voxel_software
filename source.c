...
void setScanline(vec2_t pos_1,vec2_t pos_2){
	int p_begin,p_end;
...
void drawQuad(vec2_t pos[4],pixel_t color){
	for(int i = 0;i < 4;i++){
...
void drawTriangle(vec2_t pos[3],pixel_t color){
	for(int i = 0;i < 4;i++){
		if(pos[i].x == 0.0f || pos[i].y == 0.0f)
			return;
	}
...
void generateBlockOutline(){
	...
	vec3_t block_pos;
	if(block_depth > edit_depth)
		block_pos = (vec3_t){block_pos_i.x >> (block_depth - edit_depth),block_pos_i.y >> (block_depth - edit_depth),block_pos_i.z >> (block_depth - edit_depth)};
	else
		block_pos = (vec3_t){block_pos_i.x << (edit_depth - block_depth),block_pos_i.y << (edit_depth - block_depth),block_pos_i.z << (edit_depth - block_depth)};
	vec3_t point[8];
...
void castVisibilityRays(){
	...
	for(int x = counter / 16;x < WND_RESOLUTION.x;x += 16){
		for(int y = counter % 16;y < WND_RESOLUTION.y;y += 16){
			vec3_t ray_angle = getRayAngle(x,y);
	...
	/*
	guiRectangle((vec2_t){0.0f,0.0f},(vec2_t){1.0f,1.0f},(vec3_t){1.0f,1.0f,0.0f});
	for(int x = 0;x < WND_RESOLUTION.x;x += 16){
		for(int y = 0;y < WND_RESOLUTION.y;y += 16){
			uint32_t c = traverseTreeItt(ray3CreateI(init.pos,getRayAngle(x,y)),init.node);
			guiRectangle((vec2_t){y / 16 * 0.01f - 1.0f,x / 16 * 0.01f - 1.0f},(vec2_t){0.01f,0.01f},(vec3_t){c * 0.02f,0.0f,0.0f});
		}
	}
	*/
...
void genBlockSelect(){
	...
	if(material.flags & MAT_LUMINANT){
		luminance[0] = (vec3_t){material.luminance.b,material.luminance.g,material.luminance.r};
		luminance[1] = (vec3_t){material.luminance.b,material.luminance.g,material.luminance.r};
		luminance[2] = (vec3_t){material.luminance.b,material.luminance.g,material.luminance.r};
	}
	else{
	...
	case GAMEMODE_CREATIVE:
		...
		case 1:
			...
			break;
		case 2:
			drawBlockSelect((vec3_t){0.75f,0.0f,0.0f},luminance,0.25f,0);
			drawBlockSelect((vec3_t){0.75f,0.0f,0.25f},luminance,0.25f,0);
			drawBlockSelect((vec3_t){0.75f,0.0f,0.5f},luminance,0.25f,0);
			drawBlockSelect((vec3_t){0.75f,0.0f,0.75f},luminance,0.25f,0);
			break;
		}
		break;
	case GAMEMODE_SURVIVAL:
...
void guiFrame(vec2_t pos,vec2_t size,vec3_t color,float thickness){
	...
	guiRectangle(pos,(vec2_t){thickness,size.y},color);
	guiRectangle((vec2_t){pos.x,pos.y + size.y},(vec2_t){size.x + thickness,thickness},color);
	guiRectangle((vec2_t){pos.x + size.x,pos.y},(vec2_t){thickness,size.y},color);
}
...
void drawChar(vec2_t pos,vec2_t size,char c){
	...
	texture_coord[2] = (vec2_t){t_x + (1.0f / (2048 * 3)) * 8,t_y};
	texture_coord[3] = (vec2_t){t_x + (1.0f / (2048 * 3)) * 8,t_y + (1.0f / (2048)) * 8};
...
void draw(){
	...
	for(;;){
		...
		if(in_console){
			...
			guiRectangle((vec2_t){-1.0f,0.2f},(vec2_t){0.8f,0.8f},(vec3_t){0.1f,0.2f,0.25f});
		}
		if(!context_is_gl){
			initGL(context);
		}
		for(int i = 0;i < 8;i++){
			guiFrame((vec2_t){-0.5f + i * 0.17f,-0.85f},(vec2_t){0.15f,0.15f},VEC3_ZERO,0.01f);
			guiInventoryContent(&inventory_slot[i],(vec2_t){-0.475f + i * 0.17f,-0.825f},(vec2_t){0.1f,0.1f});
		}
		for(int i = 0;i < 8;i++){
			vec3_t color;
			...
			guiFrame((vec2_t){-0.505f + i * 0.17f,-0.855f},(vec2_t){0.15f,0.15f},color,0.02f);
		}
...
/*
void y(uint32_t node_index){
	node_t* node = dynamicArrayGet(node_root,node_index);
	if(node->type == BLOCK_PARENT){
		for(int i = 0;i < 8;i++){
			node_t* child = dynamicArrayGet(node_root,node->child_s[i]);
...
#pragma comment(lib,"Winmm.lib")
	...
	triangles = tMalloc(sizeof(triangle_t) * 100000);
	texture_atlas = tMalloc(sizeof(pixel_t) * 1024 * 1024 * 2 * TEXTURE_AMMOUNT);

	bmi.bmiHeader.biWidth  = WND_RESOLUTION.y;
	bmi.bmiHeader.biHeight = WND_RESOLUTION.x;

		texture[0].size = 1;
	texture[0].data = tMalloc(1);
	...
	for(int x = 0;x < m;x++){
		for(int y = 0;y < m;y++){
			setVoxelSolid(x,y,0,depth,BLOCK_WHITE);
		}
	}
	*/
	...
	//setVoxelSolid(m * 2 + 1,m * 2 + 1,m * 2,depth + 2,BLOCK_SPAWNLIGHT);
	/*
	uint32_t n_size_bits = 4;
	uint32_t n_size = (1 << n_size_bits);
	uint32_t n_height_bits = 4;
	uint32_t n_height = (1 << n_height_bits);
	for(int x = 0;x < m;x++){
		for(int y = 0;y < m;y++){
			uint32_t c_x = x >> n_size_bits;
			uint32_t c_y = y >> n_size_bits;
			uint32_t n[4];
			...
			uint32_t c2_x = (y & n_size - 1);
			uint32_t c2_y = (x & n_size - 1);
			...
			uint32_t f = (n[0] + n[1] + n[2] + n[3]);
			for(int z = 0;z < m / 2;z++){
				if(f + 1 > z)
					setVoxelSolid(x,y,z,depth,BLOCK_WHITE);	
			}
		}	
	}
	*/
