...
void playerAttack(){
	vec3 ray_dir = getLookAngle(camera_rd.dir);
	for(int i = 0;i < ENTITY_AMMOUNT;i++){
		...
		if(!entity->type == 3)
			continue;
		float distance = rayIntersectSphere(camera_rd.pos,entity->pos,ray_dir,entity->size * 2.0f);
		if(distance < 0.0f)
			continue;
		...
		spawnNumberParticle(entity->pos,getLookAngle((vec2){camera_rd.dir.x + M_PI * 0.5f,camera_rd.dir.y}),30);
		if(entity->health < 30){
			entity->alive = false;
			return;
		}
